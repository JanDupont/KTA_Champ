...
export function analyzeGlobalClassesData() {
    let matches = JSON.parse(fs.readFileSync("data/result.json", "utf8"));
    ...
    Object.values(classes).forEach((className) => {
        ...
        Object.values(matches).forEach((match) => {
            // picks
            if (match.A.picks.includes(className) || match.B.picks.includes(className))
                classData.totalPicks++;
            // bans
            if (match.A.bans.includes(className) || match.B.bans.includes(className))
                classData.totalBans++;
            // draws
    ...
    let tableData = Object.keys(classesaData).map((className, index) => {
        ...
        let winrate = (classData.totalWins / (classData.totalWins + classData.totalLosses + classData.totalDraws) || 0) * 100;
...
export function analyzePickBanOrder() {
    let matches = JSON.parse(fs.readFileSync("data/result.json", "utf8"));
    let totalMatchesAllClasses = Object.keys(matches).length;
    let classesPickBanData = {};
    Object.values(classes).forEach((className) => {
        let classPickBanData = {
            ABan1: 0,
            BBan1: 0,
            ABan2: 0,
            BBan2: 0,
